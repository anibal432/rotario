...
try {
    ...
    // 1. Procesar imagen ANTES del INSERT para incluirla en la transacción
    $imagen_banner = null;
    ...
    $sql_check = "SELECT COUNT(*) FROM Eventos 
              WHERE Nombre_Evento = ? 
              AND Fecha_Evento = ? 
              AND Id_Tipo_Evento = ?
...
if ($stmt_check->fetchColumn() > 0) {
    throw new Exception('Este evento ya fue registrado recientemente. Espera unos segundos.');
}
    ...
    // 6. ✅ Bitácora DENTRO de la transacción, usando la variable correcta de sesión
    $sql_bitacora = "INSERT INTO Bitacora (Id_Usuario, Actividades, Fecha) VALUES (?, ?, CURDATE())";
    $stmt_bitacora = $pdo->prepare($sql_bitacora);
    $stmt_bitacora->execute([
        $_SESSION['user_id'],   // ✅ Corregido: era 'usuario_id', tu sesión usa 'user_id'
        "Creó evento: " . $_POST['nombre_evento']
    ]);

    // 7. ✅ Commit al final, cuando TODO está listo
    $pdo->commit();
